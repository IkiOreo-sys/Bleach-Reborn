...
public class EntityHollowOre extends EntityMob
{
    private static Random rand;
    
    public EntityHollowOre(final World par1) {
        this(par1, EntityHollowOre.rand.nextInt(3));
    }
    
    public EntityHollowOre(final World par1World, final int par2) {
        super(par1World);
    ...
    protected void applyEntityAttributes() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ConfigHandler.oreHP);
    ...
    protected String getHurtSound() {
        if (EntityHollowOre.rand.nextInt(100) >= 50) {
    ...
    protected String getLivingSound() {
        if (EntityHollowOre.rand.nextInt(100) >= 25) {
            return "bleachreborn:hollowscream";
        }
        return null;
    }
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        this.entityDropItem(new ItemStack(BleachItems.reiatsu, 2 + EntityHollowOre.rand.nextInt(3) + par2), 0.0f);
        if (EntityHollowOre.rand.nextInt(20 - par2) == 0) {
            this.dropItem(BleachItems.oremask, 1);
        }
    }
    ...
    static {
        EntityHollowOre.rand = new Random();
    }
}
