...
public class EntityHollowStalker extends EntityMob
{
    private static Random rand;
    
    public EntityHollowStalker(final World par1) {
        this(par1, EntityHollowStalker.rand.nextInt(3));
    }
    
    public EntityHollowStalker(final World par1World, final int par2) {
        super(par1World);
    ...
    protected void applyEntityAttributes() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ConfigHandler.stalkerHP);
    ...
    protected String getHurtSound() {
        if (EntityHollowStalker.rand.nextInt(100) >= 50) {
    ...
    protected String getLivingSound() {
        if (EntityHollowStalker.rand.nextInt(100) >= 25) {
            return "bleachreborn:hollowscream";
        }
        return null;
    }
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        this.entityDropItem(new ItemStack(BleachItems.reiatsu, 2 + EntityHollowStalker.rand.nextInt(3) + par2), 0.0f);
        if (EntityHollowStalker.rand.nextInt(20 - par2) == 0) {
            this.dropItem(Armor.StalkerHelmet, 1);
        }
    }
    ...
    static {
        EntityHollowStalker.rand = new Random();
    }
}
