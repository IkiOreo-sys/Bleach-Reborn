...
public class EntityHollowGolem extends EntityMob
{
    ...
    public EntityHollowGolem(final World par1) {
        this(par1, EntityHollowGolem.rand.nextInt(3), EntityHollowGolem.rand.nextInt(50) + 50);
    }
    
    public EntityHollowGolem(final World par1World, final int par2, final int par3) {
        super(par1World);
        this.scale = this.getRenderSize() / 100.0f;
    ...
    protected void applyEntityAttributes() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ConfigHandler.golemHP);
    ...
    protected String getHurtSound() {
        if (EntityHollowGolem.rand.nextInt(100) >= 50) {
    ...
    protected String getLivingSound() {
        if (EntityHollowGolem.rand.nextInt(100) >= 25) {
            return "bleachreborn:hollowscream";
        }
        return null;
    }
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        this.entityDropItem(new ItemStack(BleachItems.reiatsu, 2 + EntityHollowGolem.rand.nextInt(3) + par2), 0.0f);
        if (EntityHollowGolem.rand.nextInt(20 - par2) == 0) {
            this.dropItem(Armor.GolemHelmet, 1);
        }
    }
    ...
    public boolean attackEntityAsMob(final Entity par1Entity) {
        final boolean var2 = par1Entity.attackEntityFrom(DamageSource.causeMobDamage((EntityLivingBase)this), 2.0f);
        if (var2) {
            boolean attack = true;
            if (EntityHollowGolem.rand.nextInt(3) == 1) {
                attack = false;
            }
            if (attack) {
                par1Entity.motionY += 0.7000000059604645;
            }
            if (!attack) {
                par1Entity.addVelocity(this.motionX * 52.8 * 0.6000000238418579, 0.1, this.motionZ * 52.8 * 0.6000000238418579);
                par1Entity.motionY += 0.2000000059604645;
            }
    ...
    public void setRenderSize(final int par3) {
        this.dataWatcher.updateObject(21, (Object)par3);
    }
    ...
    static {
        EntityHollowGolem.rand = new Random();
    }
}
