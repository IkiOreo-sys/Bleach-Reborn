...
public class EntityHollowSnake extends EntityMob
{
    ...
    public EntityHollowSnake(final World par1) {
        this(par1, EntityHollowSnake.rand.nextInt(3));
    }
    
    public EntityHollowSnake(final World par1World, final int par2) {
        super(par1World);
    ...
    protected void applyEntityAttributes() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ConfigHandler.snakeHP);
    ...
    protected String getHurtSound() {
        if (EntityHollowSnake.rand.nextInt(100) >= 50) {
    ...
    protected String getLivingSound() {
        if (EntityHollowSnake.rand.nextInt(100) >= 25) {
            return "bleachreborn:hollowscream";
        }
        return null;
    }
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        this.entityDropItem(new ItemStack(BleachItems.reiatsu, 2 + EntityHollowSnake.rand.nextInt(3) + par2), 0.0f);
        if (EntityHollowSnake.rand.nextInt(20 - par2) == 0) {
            this.dropItem(Armor.SnakeHelmet, 1);
        }
    }
    ...
    static {
        EntityHollowSnake.rand = new Random();
    }
}
