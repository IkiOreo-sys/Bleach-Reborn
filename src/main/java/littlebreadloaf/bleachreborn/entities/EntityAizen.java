...
public class EntityAizen extends EntityMob implements IBossDisplayData
{
    private static Random rand;
    private boolean angry;
    public static final ItemStack heldItem;
    
    public EntityAizen(final World par1) {
        this(par1, EntityAizen.rand.nextInt(11));
    }
    
    public EntityAizen(final World par1World, final int par2) {
        super(par1World);
    ...
    protected void applyEntityAttributes() {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ConfigHandler.aizenHP);
            this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(160.0);
        }
        else {
            this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(15600.0);
    ...
    protected String getHurtSound() {
        if (EntityAizen.rand.nextInt(100) >= 50) {
            return null;
        }
        return null;
    }
    ...
    protected String getLivingSound() {
        if (EntityAizen.rand.nextInt(100) >= 25) {
            return null;
        }
        return null;
    }
    ...
    protected void dropFewItems(final boolean par1, final int par2) {
        ...
        this.entityDropItem(new ItemStack(BleachItems.reiatsu, 2 + EntityAizen.rand.nextInt(3) + par2), 0.0f);
        if (EntityAizen.rand.nextInt(50 - par2) == 0) {
    ...
    public void onDeath(final DamageSource par1DamageSource) {
        ...
        if (par1DamageSource.getEntity() instanceof EntityPlayer) {
            ...
            if (var8 != null && var8.getItem() == Armor.HollowHelmet && EntityAizen.rand.nextInt(20) == 0) {
                final Item recordEscalon = BleachItems.recordEscalon;
            }
            if (props.getFaction() == 3) {
                props.addSXP(8);
            }
            if (ConfigHandler.aizenSP == true) {
            	props.setMaxCap(props.getCurrentCap() + 500);
            }
        }
    }
    
    static {
        EntityAizen.rand = new Random();
