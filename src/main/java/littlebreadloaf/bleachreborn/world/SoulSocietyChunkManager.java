...
public class SoulSocietyChunkManager extends WorldChunkManager
{
    public BiomeGenBase[] SoulSocietyBiomes;
    private GenLayer genBiomes;
    private GenLayer biomeIndexLayer;
    private BiomeCache biomeCache;
    private List biomesToSpawnIn;
    
    protected SoulSocietyChunkManager() {
        this.SoulSocietyBiomes = new BiomeGenBase[] { BleachBiomes.SoulSocietyForest };
        SoulSocietyChunkManager.allowedBiomes.clear();
        SoulSocietyChunkManager.allowedBiomes.add(BleachBiomes.SoulSocietyForest);
        this.biomeCache = new BiomeCache((WorldChunkManager)this);
        (this.biomesToSpawnIn = new ArrayList()).addAll(SoulSocietyChunkManager.allowedBiomes);
    }
    
    public SoulSocietyChunkManager(final long seed, final WorldType worldType) {
        this();
    ...
    public SoulSocietyChunkManager(final World world) {
    ...
    public float[] getRainfall(float[] par1ArrayOfFloat, final int par2, final int par3, final int par4, final int par5) {
        ...
        for (int i1 = 0; i1 < par4 * par5; ++i1) {
            float f = this.SoulSocietyBiomes[aint[i1] % this.SoulSocietyBiomes.length].getIntRainfall() / 65536.0f;
            if (f > 1.0f) {
                f = 1.0f;
            }
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, final int par2, final int par3, final int par4, final int par5) {
        ...
        for (int i1 = 0; i1 < par4 * par5; ++i1) {
            par1ArrayOfBiomeGenBase[i1] = this.SoulSocietyBiomes[aint[i1] % this.SoulSocietyBiomes.length];
        }
    ...
    public boolean areBiomesViable(final int par1, final int par2, final int par3, final List par4List) {
        ...
        for (int j2 = 0; j2 < l2 * i2; ++j2) {
            final BiomeGenBase biomegenbase = this.SoulSocietyBiomes[aint[j2] % this.SoulSocietyBiomes.length];
    ...
    public ChunkPosition findBiomePosition(final int par1, final int par2, final int par3, final List par4List, final Random par5Random) {
        ...
        for (int k2 = 0; k2 < l2 * i2; ++k2) {
            ...
            final BiomeGenBase biomegenbase = this.SoulSocietyBiomes[aint[k2] % this.SoulSocietyBiomes.length];
