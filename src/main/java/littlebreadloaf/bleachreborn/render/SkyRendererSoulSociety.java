...
public class SkyRendererSoulSociety extends IRenderHandler
{
    ...
    public void render(final float partialTicks, final WorldClient world, final Minecraft mc) {
        if (world.provider.dimensionId == BleachIds.worldSoulSocietyID) {
            ...
            float f32 = 20.0f;
            FMLClientHandler.instance().getClient().renderEngine.bindTexture(SkyRendererSoulSociety.TEXTURE_MOON);
    ...
    private void renderNightSky(final float partialTicks) {
        ...
        RenderHelper.disableStandardItemLighting();
        GL11.glDepthMask(false);
        FMLClientHandler.instance().getClient().renderEngine.bindTexture(SkyRendererSoulSociety.TEXTURE_SKY);
