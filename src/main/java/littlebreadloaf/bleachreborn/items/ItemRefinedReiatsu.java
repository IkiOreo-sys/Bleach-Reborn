...
public class ItemRefinedReiatsu extends Item
{
    public final int itemUseDuration;
    
    public ItemRefinedReiatsu() {
        this.setCreativeTab(BleachItems.tabBleach);
        this.setUnlocalizedName("refined_reiatsu");
        this.itemUseDuration = 56;
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister icon) {
        this.itemIcon = icon.registerIcon("bleachreborn".toLowerCase() + ":refined_reiatsu");
    }
    ...
    public ItemStack onEaten(final ItemStack itemstack, final World world, final EntityPlayer player) {
        if (!world.isRemote) {
            ...
            if (props.getFaction() == 2) {
                props.replenishEnergy(1000);
            }
            else {
                props.replenishEnergy(700);
            }
