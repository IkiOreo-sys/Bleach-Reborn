...
public class ShikaiWater extends ItemShikai
{
    ...
    public ShikaiWater(final Item.ToolMaterial material) {
        super(material);
        this.damageBoost = 0.0f;
        this.robeboost = 0.0f;
        this.pantboost = 0.0f;
        this.sandalboost = 0.0f;
        this.waterboost = 0.0f;
        this.rainboost = 0.0f;
        this.shikaiTimer = 40;
    ...
    public String getUnlocalizedName(final ItemStack itemstack) {
        return Names.ShikaiWater_UnlocalizedName[itemstack.getItemDamage()];
    }
    ...
    public IIcon getIconFromDamage(final int damage) {
        return ShikaiWater.icons[damage];
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister icon) {
        ShikaiWater.icons = new IIcon[ShikaiWater.ICON.length];
        for (int i = 0; i < ShikaiWater.icons.length; ++i) {
            ShikaiWater.icons[i] = icon.registerIcon("bleachreborn".toLowerCase() + ":" + ShikaiWater.ICON[i]);
        }
    }
    ...
    public boolean hitEntity(final ItemStack par1ItemStack, final EntityLivingBase par2EntityLivingBase, final EntityLivingBase par3EntityLivingBase) {
        ...
        if (player.isInWater()) {
            this.waterboost = 2.0f;
        }
        else {
            this.waterboost = 0.0f;
        }
        final float damage = this.damageBoost + this.sandalboost + this.pantboost + this.robeboost + this.waterboost + this.rainboost;
    ...
    public Multimap getItemAttributeModifiers() {
        ...
        multimap.put((Object)SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), (Object)new AttributeModifier(ShikaiWater.field_111210_e, "Weapon modifier", (double)damage, 0));
        return multimap;
    }
    ...
    public void onUpdate(final ItemStack par1ItemStack, final World par2World, final Entity par3Entity, final int par4, final boolean par5) {
        if (par3Entity instanceof EntityPlayer) {
            ...
            if (heldItem != null && heldItem == par1ItemStack) {
                ...
                if (player.isInWater()) {
                    player.moveFlying(player.moveStrafing, player.moveForward, 0.05f + props.getCurrentEnergy() * props.getCurrentCap() / 10000.0f);
                }
                heldItem.setItemDamage(props.getZTex());
                player.addPotionEffect(new PotionEffect(Potion.waterBreathing.id, 40, 0));
                if (props.getZType() != 12) {
                    props.deactivate(par1ItemStack.getItem());
                }
            }
        }
    ...
    static {
        ICON = Names.ShikaiWater_UnlocalizedName;
    }
}
