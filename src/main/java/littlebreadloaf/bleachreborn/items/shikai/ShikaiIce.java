...
public class ShikaiIce extends ItemShikai
{
    ...
    public ShikaiIce(final Item.ToolMaterial material) {
        super(material);
    ...
    public IIcon getIconFromDamage(final int damage) {
        return ShikaiIce.icons[damage];
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister icon) {
        ShikaiIce.icons = new IIcon[ShikaiIce.ICON.length];
        for (int i = 0; i < ShikaiIce.icons.length; ++i) {
            ShikaiIce.icons[i] = icon.registerIcon("bleachreborn".toLowerCase() + ":" + ShikaiIce.ICON[i]);
        }
    }
    ...
    public boolean hitEntity(final ItemStack par1ItemStack, final EntityLivingBase par2EntityLivingBase, final EntityLivingBase par3EntityLivingBase) {
        ...
        final float damage = this.damageBoost + this.sandalboost + this.pantboost + this.robeboost + this.coldboost;
    ...
    public Multimap getItemAttributeModifiers() {
        ...
        multimap.put((Object)SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), (Object)new AttributeModifier(ShikaiIce.field_111210_e, "Weapon modifier", (double)damage, 0));
        return multimap;
    }
                ...
                for (int i = -this.shockwaveRing; i < this.shockwaveRing; ++i) {
                    for (int k = -this.shockwaveRing; k < this.shockwaveRing; ++k) {
                        if (Math.ceil(par3Entity.getDistance(par3Entity.posX + i, par3Entity.posY, par3Entity.posZ + k)) == this.shockwaveRing) {
                            PacketDispatcher.sendToAll((IMessage)new ParticleMessage(8, this.Xpos + i, this.Ypos, this.Zpos + k));
                        }
                    }
                }
                if (this.ringTimer <= 0) {
    ...
    static {
        ICON = Names.ShikaiIce_UnlocalizedName;
    }
}
