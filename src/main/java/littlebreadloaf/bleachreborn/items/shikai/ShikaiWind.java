...
public class ShikaiWind extends ItemShikai
{
    ...
    public ShikaiWind(final Item.ToolMaterial material) {
        super(material);
    ...
    public IIcon getIconFromDamage(final int damage) {
        return ShikaiWind.icons[damage];
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister icon) {
        ShikaiWind.icons = new IIcon[ShikaiWind.ICON.length];
        for (int i = 0; i < ShikaiWind.icons.length; ++i) {
            ShikaiWind.icons[i] = icon.registerIcon("bleachreborn".toLowerCase() + ":" + ShikaiWind.ICON[i]);
        }
    }
    ...
    public boolean hitEntity(final ItemStack par1ItemStack, final EntityLivingBase par2EntityLivingBase, final EntityLivingBase par3EntityLivingBase) {
        ...
        if (!player.worldObj.isRemote) {
            par2EntityLivingBase.motionY += 0.4000000060607645;
        }
    ...
    public Multimap getItemAttributeModifiers() {
        ...
        multimap.put((Object)SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), (Object)new AttributeModifier(ShikaiWind.field_111210_e, "Weapon modifier", (double)damage, 0));
        return multimap;
    }
    ...
    public void onUsingTick(final ItemStack stack, final EntityPlayer player, final int count) {
        ...
        final List list = player.worldObj.getEntitiesWithinAABBExcludingEntity((Entity)player, player.boundingBox.copy().expand(Math.abs(normalizer.xCoord * 3.0) + 3.0, Math.abs(normalizer.yCoord * 3.0) + 3.0, Math.abs(normalizer.zCoord * 3.0) + 3.0));
        ...
        if (!player.worldObj.isRemote) {
            if (this.windSound <= 0) {
                player.worldObj.playSoundAtEntity((Entity)player, "bleachreborn:wind", 0.4f, 1.0f + this.rand.nextFloat());
                this.windSound = 30;
            }
    ...
    static {
        ICON = Names.ShikaiWind_UnlocalizedName;
    }
}
