...
public class ItemReiatsuPill extends Item
{
    public final int itemUseDuration;
    
    public ItemReiatsuPill() {
        this.setCreativeTab(BleachItems.tabBleach);
        this.setUnlocalizedName("reiatsu_pill");
        this.itemUseDuration = 48;
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister icon) {
        this.itemIcon = icon.registerIcon("bleachreborn".toLowerCase() + ":reiatsu_pill");
    }
    ...
    public ItemStack onEaten(final ItemStack itemstack, final World world, final EntityPlayer player) {
        if (!world.isRemote) {
            ...
            if (props.getFaction() == 2) {
                props.replenishEnergy(100);
            }
            else {
                props.replenishEnergy(75);
            }
