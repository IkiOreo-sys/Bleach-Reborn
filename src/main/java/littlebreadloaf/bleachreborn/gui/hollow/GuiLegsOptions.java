...
public class GuiLegsOptions extends GuiScreen
{
    ...
    public GuiLegsOptions(final EntityPlayer player) {
        this.texture = new ResourceLocation("bleachreborn".toLowerCase(), "textures/guis/legs_select_gui.png");
        this.id = -1;
        this.choice = 1;
        this.points = 0;
        ...
        switch (button.id) {
            ...
            case 3: {
                if (ExtendedPlayer.get((EntityPlayer)this.player).getCurrentHPoints() >= this.points || this.player.capabilities.isCreativeMode) {
                    PacketDispatcher.sendToServer((IMessage)new HollowPieceMessage(4, this.choice, this.points));
