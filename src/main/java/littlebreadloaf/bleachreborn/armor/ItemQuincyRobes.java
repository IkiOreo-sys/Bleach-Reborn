...
public class ItemQuincyRobes extends ItemArmor
{
    ...
    public ItemQuincyRobes(final ItemArmor.ArmorMaterial par2, final int par3, final int par4) {
        super(par2, par3, par4);
        this.replenishTimer = 200;
        this.setCreativeTab(BleachItems.tabBleach);
        if (par4 == 1) {
            this.setTextureName("bleachreborn:quincytop");
        }
        else if (par4 == 2) {
            this.setTextureName("bleachreborn:quincyslacks");
        }
        else if (par4 == 3) {
            this.setTextureName("bleachreborn:quincyshoes");
        }
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister par1RegisterIcon) {
        if (this == Armor.QuincyRobe) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":quincytop");
        }
        if (this == Armor.QuincyPants) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":quincyslacks");
        }
        if (this == Armor.QuincyShoes) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":quincyshoes");
        }
    }
    
    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String layer) {
        if (stack.getItem() == Armor.QuincyRobe || stack.getItem() == Armor.QuincyShoes) {
            return "bleachreborn".toLowerCase() + ":textures/models/armor/quincy_1.png";
        }
        if (stack.getItem() == Armor.QuincyPants) {
            return "bleachreborn".toLowerCase() + ":textures/models/armor/quincy_2.png";
        }
        return null;
    }
    ...
    public void onArmorTick(final World world, final EntityPlayer player, final ItemStack itemStack) {
        if (!world.isRemote) {
            ...
            if (this.replenishTimer <= 0 && props.getFaction() == 2) {
