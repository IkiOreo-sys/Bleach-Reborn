...
public class ItemHogyokuArmor extends ItemArmor
{
    int replenishTimer;
    
    public ItemHogyokuArmor(final ItemArmor.ArmorMaterial par2, final int par3, final int par4) {
        super(par2, par3, par4);
        this.replenishTimer = 200;
        this.setCreativeTab(BleachItems.tabBleach);
        if (par4 == 1) {
            this.setTextureName("bleachreborn:hogyoku_top");
        }
        else if (par4 == 2) {
            this.setTextureName("bleachreborn:hogyoku_pants");
        }
        else if (par4 == 3) {
            this.setTextureName("bleachreborn:hogyoku_shoes");
        }
    }
    
    @SideOnly(Side.CLIENT)
    public void registerIcons(final IIconRegister par1RegisterIcon) {
        if (this == Armor.HogyokuJacket) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":hogyoku_top");
        }
        if (this == Armor.HogyokuPants) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":hogyoku_pants");
        }
        if (this == Armor.HogyokuShoes) {
            this.itemIcon = par1RegisterIcon.registerIcon("bleachreborn".toLowerCase() + ":hogyoku_shoes");
        }
    }
    
    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String layer) {
        if (stack.getItem() == Armor.HogyokuJacket || stack.getItem() == Armor.HogyokuShoes) {
            return "bleachreborn".toLowerCase() + ":textures/models/armor/hogyoku_armor_1.png";
        }
        if (stack.getItem() == Armor.HogyokuPants) {
            return "bleachreborn".toLowerCase() + ":textures/models/armor/hogyoku_armor_2.png";
        }
        return null;
    }
